...
func (d *DataInfoClient) CreateGroup(ctx context.Context, name string, members []string) (Group, error) {
	...
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, uri, bytes.NewBuffer(msg))
	if err != nil {
		return g, errors.Wrap(err, "Failed to create request to create group")
	}
	req.Header.Set("content-type", "application/json")

	resp, err := httpClient.Do(req)
	if err != nil {
		return g, errors.Wrap(err, "Failed in request to data-info")
...
func (d *DataInfoClient) UpdateGroupMembers(ctx context.Context, name string) (Group, error) {
	var g Group
	// PUT /:group-name/
	return g, nil
}
